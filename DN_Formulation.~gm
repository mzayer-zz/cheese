...
L all lines /l1*l10/
L_ex(L)  existing-Lines /l1*l4/
L_can(L)   candidate-Lines /l5*l10/
...
Scalars M big number /1000000/
Vref /13.8/
Lf Loss factor just for test now /0.7/
LossCost_C just for test now /2500/
LOLEcost just for test /150000000/
...
eq_InvCost..  InvCost =e= sum((S_can,t), (Z1(S_can,t)-Z1(S_can,t-1))*node_data(S_can,'NIC1') + (Z2(S_can,t)-Z2(S_can,t-1))*node_data(S_can,'NIC2'))
+sum((S_ex,t),(ZR(S_ex,t)-ZR(S_ex,t-1))*node_data(S_ex,'NRC'))+ sum((L_ex,t),(XR(L_ex,t)-XR(L_ex,t-1))*Line_data(L_ex,"LRC"))
+ sum((L_can,t),(XI(L_can,t)-XI(L_can,t-1))*Line_data(L_can,"LIC")) ;
...
KCL1(S,t).. Pprod(S,t) =e= sum(L$(Line_data(L,'fbus')=ord(S)),PF(L,t)) ;
*KCL2(S,t).. sum(L$(Line_data(L,'tbus')=ord(S)),QF(L,t)) + Qprod(S,t) =e= sum(L$(Line_data(L,'fbus')=ord(S)),QF(L,t)) ;
KCL2(S,t).. Qprod(S,t) =e= sum(L$(Line_data(L,'fbus')=ord(S)),QF(L,t)) ;
KCL3(np,t).. sum(L$(Line_data(L,'tbus')=card(S)+ord(np)),PF(L,t)) =e= sum(L$(Line_data(L,'fbus')=card(S)+ord(np)),PF(L,t)) + LoadP_data(np,t) - r(np,t) ;
KCL4(np,t).. sum(L$(Line_data(L,'tbus')=card(S)+ord(np)),QF(L,t)) =e= sum(L$(Line_data(L,'fbus')=card(S)+ord(np)),QF(L,t)) + LoadQ_data(np,t) - r(np,t) ;


*KVL:   constraints are designed in a way to be relaxed when line is not in use.
* A /v1 is missing in KVL
KVL1(L_ex,t).. sum(n,A(L_ex,n)*V(n,t)) =e= -(PF(L_ex,t)*Line_data(L_ex,"R") + QF(L_ex,t)*Line_data(L_ex,"X"))/Vref ;
...
eq_referenceV(S,t).. V(S,t) =e= Vref ;
*eq_Vmax(np,t).. V(np,t) =l= Node_data(np,"Vmax") ;
*eq_Vmin(np,t).. V(np,t) =g= Node_data(np,"Vmin") ;
...
*Radiality1(t).. 3 + sum(L_can, XI(L_can,t)) =e= sum(S_can,Z1(S_can,t) + Z2(S_can,t)) + number_of_nodes(t) - 1 ;
Radiality2(L_can,t).. sum((i,j)$((ord(i)=Line_data(L_can,'tbus')) and (ord(j) = Line_data(L_can,'fbus'))), B(i,j,t) + Bp(j,i,t) ) =e= XI(L_can,t) ;
Radiality3(i,t).. sum(j$( (ord(j)=Line_data(L,'fbus')) and (ord(i)=Line_data(L,'tbus')) ) ,B(i,j,t)) =e= 1 ;
Radiality4(S,j,t).. B(S,j,t) =e= 0
display LoadP_data
